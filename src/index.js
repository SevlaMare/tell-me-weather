...
// const api = '6ca8aaad293b0b0a3ad93f29e82425f87x2323414sadasaaaaa';
...
const safeFetch = (url) => {
  try { return fetchData(url) }
  catch(err) { return console.warn('Bad request |', err) }
}
...
sel('#btnSend').addEventListener('click', (event) => {
  ...
  safeFetch(url)
    .then( (info) => {render(info.name, info.temperature, info.weather)})
